...
import sys
import time
from all_logging.log_scripts import common_logging
import logging
import json
import threading
...
sys.path.append('../')
from library.functions import decode_and_send, listen_and_get
from library.variables import *
...
class ClientTransport(threading.Thread, QObject):
    ...
    def connection_init(self, port, ip):
        ...
        try:
            with socket_lock:
                decode_and_send(self.transport, self.create_presence())
                self.process_server_ans(listen_and_get(self.transport))
    ...
    def contacts_list_update(self):
        ...
        with socket_lock:
            decode_and_send(self.transport, req)
            ans = listen_and_get(self.transport)
    ...
    def transport_shutdown(self):
        ...
        with socket_lock:
            try:
                decode_and_send(self.transport, message)
            except OSError:
                pass
    ...
    def send_message(self, to, message):
        ...
        with socket_lock:
            decode_and_send(self.transport, message_dict)
            self.process_server_ans(listen_and_get(self.transport))
        ...
        while self.running:
            ...
            with socket_lock:
                try:
                    ...
                    message = listen_and_get(self.transport)
                except OSError as err:
                    if err.errno:
                        logger.critical(f'Потеряно соединение с сервером.')
                        self.running = False
